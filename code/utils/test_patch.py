...
def var_all_case(model, num_classes, patch_size=(64,64,64), stride_xy=4, stride_z=4, dataset_name="../../dataset"):
    img_nib_path=[]
    label_nib_path=[]
    if dataset_name == "ISLES22":
        for i in range(201, 251):
            img_nib_path.append(dataset_name+"/ISLES_2022/Sample_"+ str(i)+'/dwi_case.nii.gz')
            label_nib_path.append(dataset_name + "/ISLES_2022/Sample_" + str(i)+'/label_case.nii.gz')

    loader = tqdm(zip(img_nib_path,label_nib_path))
    total_dice = 0.0
    for image_path,label_path in loader:
        sample = {'image': image_path, 'label': label_path}
        sample1 = dataloaders.isles2022_process.get_val_transform()(sample)
        image = sample1['image'].as_tensor()
        label = sample1['label'].as_tensor()
