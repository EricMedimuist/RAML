...
class Decoder_Contour(nn.Module):
    ...
    def forward(self, features):
        ...
        out_7 = x7
        if self.has_dropout:
            out_7 = self.dropout(out_7)
        out_7 = self.out_conv_7(out_7)
        ...
        out_8 = x8
        if self.has_dropout:
            out_8 = self.dropout( out_8)
        out_8 = self.out_conv_8( out_8)
...
if __name__ == "__main__":
    model=CACNet3d(n_channels=1, n_classes=2, normalization='batchnorm', has_dropout=True)
    model = model.cuda()
